...
def set_optim(opt, model):
    ...
    if opt.scheduler == 'fixed':
        ...
        if opt.scheduler_steps is None:
            scheduler_steps = opt.total_steps / opt.accumulation_steps
        else:
            scheduler_steps = opt.scheduler_steps / opt.accumulation_steps
        scheduler = WarmupLinearScheduler(optimizer, warmup_steps=opt.warmup_steps / opt.accumulation_steps, scheduler_steps=scheduler_steps, min_ratio=0., fixed_lr=opt.fixed_lr)
