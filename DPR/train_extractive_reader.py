...
class ReaderTrainer(object):
    ...
    def get_data_iterator(
        ...
        # #     for pasg in sample.passages:
        # #         len_count[len(pasg.sequence_ids)] += 1
        # with open('/home/t-kaixinma/DPR/downloads/data/retriever_results/nq/single/test.json', 'r') as f:
        #     original_data = json.load(f)
        # self.tensorizer.set_pad_to_max(False)
        # for sample in tqdm(original_data):
        #     #qids = self.tensorizer.text_to_tensor(sample['question'], add_special_tokens=False)
        #     #len_count[len(qids)] += 1
        #     for p in sample['ctxs']:
        #        token_ids = self.tensorizer.text_to_tensor(p['text'], add_special_tokens=False)
        #        len_count[len(token_ids)] += 1
        # print (len_count)
        # exit(0)
    ...
    def validate(self):
        ...
        for n in sorted(ems.keys()):
            ...
            logger.info("number of examples=%d" % (len(ems[n])))
