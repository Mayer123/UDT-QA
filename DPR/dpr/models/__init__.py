...
def init_hf_cos_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError('Please install transformers lib')
    from .hf_models_cos import get_any_biencoder_components
    return get_any_biencoder_components(args, **kwargs)
...
def init_hf_any_reader(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError('Please install transformers lib')
    from .hf_models import get_any_reader_components
    return get_any_reader_components(args, **kwargs)
...
BIENCODER_INITIALIZERS = {
    ...
    'hf_cos': init_hf_cos_biencoder,
}

READER_INITIALIZERS = {
    'hf_bert': init_hf_bert_reader,
    'hf_any': init_hf_any_reader,
}
