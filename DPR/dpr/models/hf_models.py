...
class HFEncoder(nn.Module):
    def __init__(self, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, **kwargs):
        super().__init__()
    ...
    def forward(
        ...
        if self.encoder.config.output_hidden_states:
            sequence_output, pooled_output, hidden_states = self.encoder.forward(
                input_ids=input_ids,
            ...
            hidden_states = None
            #sequence_output, pooled_output = self.encoder.forward(
