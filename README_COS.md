...
Our multi-task finetuned COS model and pretrained COS model can be downloaded. Note that the pretrained model only has 4 experts. 
...
# For entity span proposal 

2 for entity query, 3 for entity index, 4 for span query, 5 for span index
...
For each dataset, we used their corresponding knowledge sources, and they can be downloaded using the following commands:
```
```
...
If you would like to train your own COS model, we provide the training data for COS, you can download them using download_data.py (resource key: core.data.retriever). Note that you will also need to update the corresponding file paths in /DPR/conf/datasets/encoder_train_default.yaml.

To train the OTT-QA and NQ joint retriever,
```
python -m torch.distributed.launch --nproc_per_node=16 python train_dense_encoder_COS.py train=biencoder_nq  train_datasets=[hotpot_train_iter2,hotpot_train_hop2_iter2,hotpot_question_link_train_iter2,hotpot_link_train_iter2,hotpot_train_rerank_iter2,ott_hop1_train,ott_hop2_train_iter2,ott_link_train_iter2,ott_rerank_train_iter2,NQ_train_iter2,NQ_train_table_iter2,NQ_train_rerank_iter2_title,NQ_train_table_rerank_iter2_title] dev_datasets=[hotpot_dev_iter2,hotpot_dev_hop2_iter2,hotpot_question_link_dev_iter2,hotpot_link_dev_iter2,hotpot_dev_rerank_iter2] val_av_rank_start_epoch=35 output_dir=/home/v-kaixinma/tmp train.batch_size=12 global_loss_buf_sz=1184000 train.val_av_rank_max_qs=20000 train.warmup_steps=8000 encoder.pretrained_file=/home/v-kaixinma/laketahoe_checkpoints/taser_pretrain/biencoder_taser_mod2_attn_n4_encoder_blink_contriever_mdr_2xG16_v2_from_contriever_v2/dpr_biencoder.19 checkpoint_file_name=dpr_biencoder encoder.use_moe=True encoder.num_expert=6 encoder.moe_type=mod2:attn train.use_layer_lr=False train.learning_rate=2e-5 encoder.encoder_model_type=hf_cos
```
After training is done, you can generate embeddings for the knowledge sources using the following command (for ctx_src, use ott_table for OTT-QA and dpr_wiki for NQ)
```
...
## Reader Evaluation 
We provide the trained FiE reader model for NQ and OTT-QA respectively, as well as the reader data
...
If you would like to train your own FiE reader model, just launch with the following command and add the --do_train flag
```
